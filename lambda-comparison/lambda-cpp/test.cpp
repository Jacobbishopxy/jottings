...
void simple_capture()
{
  ...
  auto rise_headless_horseman{[data](std::string_view str)
                              { std::cout << data << " " << str; }};
...
TEST_CASE("lambda-comparison")
{
  lambda_and_fn_ptr();
  simple_lambda1();
  simple_lambda2();

  passing_lambda_to_fn([](int i)
                       { return i * 2; });
